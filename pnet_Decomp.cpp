...
//		share->NetCBal = share->NetCBal - share->SoilDecResp; // updating NetCBal
	...
	//	fNRatioNit = 1/30.4;  // daily step; Linghui Meng 20201221
...
//		site->Kden = 0.13; //* share->dayspan;//0.025*30=mon-1,  default 0.03 (day-1) (McCray,2005)  // for HB
		...
		//Plant Uptake //adjust rootNsinkeff for special case
		if (share->PlantN >= veg->MaxNStore) {
			share->RootNSinkEff = 0;
		}
		else {
			share->RootNSinkEff = sqrt(1 - (share->PlantN / veg->MaxNStore));   // this is based on monthly rate move from allocateYr to Decomp
		}
		
		RootNSinkStr = share->RootNSinkEff * TMult;
		if (RootNSinkStr > 1)RootNSinkStr = 1;
//		if (RootNSinkStr > 0.98)RootNSinkStr = 0.98;
		RootNSinkStr = RootNSinkStr*(clim->timestep / 30.4);// *1.5 to make daily to the same as monthly;  // daily step
