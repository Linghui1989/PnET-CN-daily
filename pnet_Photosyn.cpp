...
void pnet_model::Photosyn(site_struct* site, veg_struct* veg, clim_struct* clim, int rstep, share_struct* share)
{
	...
	double LayerResp, LayerNetPsn, netPsnumol, RelLayer, RelO3, LayerG, LayerDO3, LayerNetPsnO3, LayerGrossPsnO3, CanopyNetPsnPot;
...
//	PsnTMax = 35; //Linghui 05292023 set the maximum temperature for photosynthesis
	...
	if ((clim->tmin[rstep] < 6) && (DTemp > 0) && (share->GDDTot >= veg->GDDFolEnd))
	{
		...
		DTemp = DTemp * (1.0 - ((6.0 - clim->tmin[rstep]) / 6.0));  // need to verify zzx
	}
...
//	share->EnvMaxFol <-DTemp; //Linghui Meng 20210425
	...
	//Set atmospheric CO2 concentrationW 
	Ca = clim->CO2[rstep];
	...
	GrossAmax = Amax *(1 + veg->BaseFolRespFrac);
//	GrossAmax = (GrossAmax * share->DVPD * DTemp * share->DayLength * 12.0) / ten9;  //g C/g Fol/day, 12 for C atom 

//	share->EnvMaxFol = share->EnvMaxFol + DTemp;	//Linghui Meng 20200518
//	share->SppMaxFol = share->SppMaxFol + share->DVPD; //Linghui Meng 20200518

	GrossAmax = (GrossAmax * DTemp * share->DayLength * 12.0) / ten9; //Linghui 05292023
	...
	if (share->FolMass > 0)
	{
		...
		iFolMass = share->FolMass / nLayers;
		ix = 0;  //
		...
		while (1)
		{
			...
			LayerGrossPsnO3 = LayerGrossPsn * LayerDO3; //Linghui Meng 20210127 O3 effect on GrossPsn
