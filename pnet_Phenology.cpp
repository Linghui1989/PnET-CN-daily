...
void pnet_model::Phenology(veg_struct* veg, clim_struct* clim, int rstep, share_struct* share, int GrowthPhase)
{
	...
	{
		share->FolLitM = 0;
		if ((share->PosCBalMass < 0.95*share->FolMass) && (clim->doy[rstep] > veg->SenescStart)) //assume the sensence occur when photosynthesis less than 95% of the C need for maintain Linghui 20240723
		{
