...
void pnet_model::WriteoutYr_netcdf(clim_struct* clim, out_struct* out, char* filename)
{
	int nyrs = clim->year[clim->length] - clim->year[1] + 1;
	int startyr = clim->year[1];
	...
	fprintf(fileoutY, "year,   folm,  deadwoodm,  livewoodm,  rootm, som, son,"
		...
		"v1, v2,v3"
		"\n");
	fprintf(fileoutY, "year,    g/m2,   g/m2,        g/m2,     g/m2,    g/m2,      gN/m2,   "  //Linghui 0729 add "year"
	...
	for (int i = 1; i < nyrs + 1; i++)
	{
		...
		fprintf(fileoutY, "%7.1f, %6.1f, %6.1f, %6.1f, %6.1f, %6.2f,%6.2f,", out->prec[i], out->evap[i],
			out->trans[i], out->et[i], out->drain[i],out->waterstress[i] ,out->soilwater[i]);// water cycle
...
void pnet_model::WriteoutDay(site_struct* site, veg_struct* veg, clim_struct* clim, share_struct* share, int rstep, FILE*& fileoutM)
{
	...
	if (iheader == 1)
	{
		...
		fprintf(fileoutM, " year,   doy,  temp,   GrowthDD,  "
			...
			"LAI,  LightEffMin, LightEffCBal, "
			"NetPsnMo, GrsPsnMo, NEE,    "
			"WoodMR,   WoodGR,   FolGR,   RootGR,   RootMR, SoilResp, WoodDecResp,"   //Linghui 0729 add wooddecresp
			"airT, NetNMinYr, NetNitrYr,NRatioNit,NDrain,NDrainMgL,FoliarN,"
			"N2O,     NO,      N2,  FluxN2OYrDe, FluxNOYrDe,"
			"SON, plnatN,litterN,DVDP" //Linghui 20200511
			"\n");

		fprintf(fileoutM, "year, day, oC, oC,"
			"cm, kpa, cm, cm, cm, cm,"
			"Index,   Days,   total,   "
			"g/m2,     g/m2,     g/m2,      g/m2,   "
			"gC/m2,     gC/m2,    gC/m2,    gC/m2,    "
			...
			"m2/m2,         gC/m2,    "
			"gC/m2,     gC/m2,    gC/m2,   "
			"gC/m2, gC/m2, gC/m2, gC/m2, gC/m2, gC/m2, gC/m2," //Linghui 0729 add wooddecresp
			"oC, gN/m2, gN/m2, gN/m2, gN/m2, mgN/l, %%,"
			"mgN/m2,mgN/m2,mgN/m2,mgN/m2,mgN/m2,"  // Ngas
			"g/m2, gN/m2, %%, %%" //Linghui 20200511
			"\n");
	...
	fprintf(fileoutM, "%8.2f, %8.2f, %8.2f, ",
		...
		share->GrsPsnMo,
		-share->NetCBal);// photosynthesis
	fprintf(fileoutM, "%8.1f, %8.1f, %8.1f, %8.1f, %8.1f, %8.1f, %8.1f,",
		share->WoodMRespYr,
		share->WoodGRespYr,
		share->FolGRespYr,
		share->RootGRespYr,
		share->RootMRespYr,
		share->SoilDecRespYr,
		share->WoodDecRespYr);    //Linghui 0729 add wooddecresp
	...
	fprintf(fileoutM, "%8.2f, %8.2f, %8.2f, %8.2f,",
		share->PlantN,	//Linghui 20200511
		share->WoodMassN,
		share->RootNSinkEff,
		share->NO3); //Linghui 20200511
...
int pnet_model::Disturbance(site_struct* site, veg_struct* veg, share_struct* share, int scenario)
{
	...
	if (scenario == 1) //HB W6
	{
		...
		site->disestart = 1996;
		site->disestop = 2010;
		site->distintens = 0.016;


		site->distyear[5] = 2016;
		site->distintensity[5] = 0.09;
		site->distremove[5] = 0.0;
	...
	if (scenario == 2) //HF
	{
		...
		site->distyear[1] = 1938;   //hurricane salvage
		site->distintensity[1] = 0.6;   //mortality
		site->distremove[1] = 0.4;		//
		site->distsoilloss[1] = 0.0;  //soil remove
		site->folregen[1] = 300;       //Linghui Meng 20200507 change the value from 100 to 300

		
		site->yrurstart = 1950; //the year urbanization started
		site->yrurstart2 = 2020;
	...
	if (scenario == 3) //HB w5
	{
		...
		site->distyear[5] = 1983;
		site->distintensity[5] = 0.92;
		site->distremove[5] = 0.87;
		site->distsoilloss[5] = 0.0;
		site->folregen[5] = 100;
	}
